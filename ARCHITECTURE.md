...
- **Per-job rerun**: a failed row reveals a rerun button while hovered (`Opacity` driven by a `DataTrigger` on the row `Border`'s `IsMouseOver` via `RelativeSource AncestorType=Border`; `Visibility` is bound to `CanRerun`, so passing rows reserve no space while failed rows reserve it whether hovered or not — the duration column never shifts sideways). `CheckRerun_Click` calls `GitHubService.RerunJobAsync(owner, repo, jobId)` → `POST repos/{o}/{r}/actions/jobs/{id}/rerun`, which restarts that job and its dependents only, unlike `RerunFailedJobsAsync` (whole run) behind the row context menu. `MainWindow._rerunningJobs` holds in-flight job ids so a second click is ignored while the panel still shows the old state. Success notifies and reloads the panel; a refusal surfaces GitHub's own stderr in a `DarkMessageBox`.
