...
- **`ShouldAutoRefresh(result)`** requires all three: `Status == Ok`, at least one check with `IsInProgress` (running or queued), and `RateLimitRemaining ?? int.MaxValue >= MinRateLimitRemaining` (100). A finished PR, a failed call, a rate-limited call or a thin budget each simply end the cycle.
- **Budget awareness**: `PrChecksQuery` asks for `rateLimit { remaining resetAt }`, which costs no extra points. `ParseRateLimitBudget` reads it, so the panel backs off *before* GitHub starts refusing calls and leaves the remaining budget for polling and manual actions. At one point per reload, a continuously watched running build costs ~120 of 5000 points per hour.
