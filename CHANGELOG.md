...
### Added
- **Label chips and a priority indicator on PR rows.** PR Monitor now fetches each PR's GitHub labels and lets you map them to small chips shown next to `repo #number`. A rule on the new **Labels** settings tab maps a label name (case-insensitive) to a chip text, a colour picked from a palette, the Windows colour dialog (**Custom colour…**) or **Default**, which makes the chip follow the row's CI status colour and a **Priority** dropdown: **None** (chip only), **High** (a left accent bar in the CI status colour, and the PR moves to the top of its section) or **Low** (the PR moves to the bottom of its section). Within each group the existing order is kept, High wins when a PR matches both, and the Stacks section keeps its chain order. The default rules show `Prioriteit/High` as a `HIGH` chip with High priority, and `Prioriteit/Low` as a muted grey (`#8B949E`) `LOW` chip with Low priority. All of a PR's labels, mapped or not, are listed in the row tooltip. The Settings window is slightly wider so all tabs still fit on one row, and the **Add label rule** button has horizontal padding so its text is not cramped.
...
- **CI checks panel — see which jobs run, passed or failed without leaving the app.** Click the colored status dot of a PR row (or **Show CI checks** in its right-click menu) to open a panel over the list with every check on the PR's latest commit. A failing PR no longer means clicking through to GitHub, its checks tab and the workflow run to find out which job broke.
  ...
  - A summary line reads `CHECKS RUNNING 3/4`, `2 CHECKS FAILED` or `ALL CHECKS PASSED`.
  - **Skipped checks are hidden** behind a `Show 3 skipped checks` line, and identical repeated runs are merged into one row with a `×N` badge — a workflow that triggers on every review would otherwise bury the interesting checks under a dozen copies of the same skipped job.
  - **Hovering a failed job replaces its duration with a rerun button** that restarts that job alone, where the row context menu's **Rerun failed jobs** restarts every failed job on the PR. The job stays on screen as queued while GitHub builds the new attempt.
  - **The panel reloads itself every 30 seconds while jobs are running**, and the refresh button shows whether it is doing so. The cycle stops as soon as everything has finished, when the panel closes, or when a reload fails.
  - Checks are fetched only while the panel is open, so polling is unchanged. Every GraphQL query now also asks GitHub for the remaining rate-limit budget — a field that costs nothing — and the panel paces itself against it, slowing down and finally stopping instead of running into the limit.

### Changed
- **Idle memory usage roughly halved (working set ~346 MB → ~157 MB, private bytes ~244 MB → ~63 MB).** WPF spun up a Direct3D device at startup and mapped the entire graphics-driver stack — on an Intel GPU some 150 MB of driver DLLs, the 66 MB shader compiler among them, plus their private heaps and about 20 driver worker threads — into a process that never draws a single pixel on the GPU: the PR window uses `AllowsTransparency`, so WPF already rasterizes it in software. The app now opts out of hardware rendering explicitly (`RenderOptions.ProcessRenderMode = SoftwareOnly`), keeping that stack out of the process entirely. Thread count drops from ~41 to ~19, and the driver heaps that grew over long sessions are gone with it. Rendering is unchanged — rounded corners, transparency and drop shadows all still work, because that window was never GPU-rendered to begin with.

### Fixed
- **The Later section no longer flashes on startup before its PRs are reconfirmed.** Its count was restored from settings and shown immediately while the window was still in its initial loading state, even though a saved Later PR may since have been merged, closed, or unsnoozed. The section now stays hidden until the first poll completes.
