...
- **A rerun button appears on a failed job when you hover its row.** It reruns that one job (and whatever depends on it) via `POST /repos/{owner}/{repo}/actions/jobs/{id}/rerun`, instead of restarting every failed job on the PR the way the row context menu's **Rerun failed jobs** does. It shows only on failed or cancelled GitHub Actions jobs — passing jobs, legacy status contexts and third-party checks have nothing to rerun. After a successful request the panel reloads (so the job flips to queued and the auto-refresh picks it up) and a toast confirms it; a refusal is reported with GitHub's own message. A second click while the request is in flight is ignored.
