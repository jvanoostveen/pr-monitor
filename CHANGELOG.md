...
- **The refresh button shows whether auto-refresh is on**, the way the header's pin button shows its state: a blue "autorenew" glyph with the current interval next to it while the panel updates itself, a grey "refresh" glyph when a click is needed. Its tooltip spells out which of the two applies (`Auto-refresh is on — reloading every 30s while jobs are running` / `Auto-refresh is off — no jobs are running`). Clicking it always reloads immediately.
- **Check rows now name the workflow they come from**, written the way GitHub writes it — `Components / Test` with the workflow dimmed in front of the job name. A bare job name is routinely ambiguous: two workflows each having a `Test` job produced two identical-looking rows with no way to tell which one had failed.
- **A rerun button appears on a failed job when you hover its row**, taking the place of the duration so the row neither widens nor grows taller. It reruns that one job (and whatever depends on it) via `POST /repos/{owner}/{repo}/actions/jobs/{id}/rerun`, instead of restarting every failed job on the PR the way the row context menu's **Rerun failed jobs** does. It shows only on failed or cancelled GitHub Actions jobs — passing jobs, legacy status contexts and third-party checks have nothing to rerun. After a successful request the panel reloads and a toast confirms it; a refusal is reported with GitHub's own message. A second click while the request is in flight is ignored.
- **The job you rerun stays on screen.** GitHub creates a new attempt behind the scenes, and for a moment the job is either missing from the check rollup or reported as skipped because its previous check run was superseded — so the row vanished the instant you clicked rerun. It is now shown as queued until GitHub reports the restarted run, for at most the grace period below.
- **Auto-refresh restarts by itself after a rerun.** GitHub needs a moment to flip a restarted job to queued, so the reload straight after a rerun would otherwise still see only finished checks and stop the cycle — leaving the panel stale exactly when it should be watching. A two-minute grace period keeps it reloading until the job actually appears, after which the normal rule takes over. The grace never overrides a failed call or a rate limit, and it is cleared when the panel closes or another PR is opened.
- The job list now uses shared column widths, so durations line up across every row regardless of which rows carry a `×N` badge or a rerun button.
- **Skipped checks are hidden by default.** A job that never ran says nothing about the PR, and a review-triggered workflow can contribute a dozen of them. A `Show 3 skipped checks` line under the list reveals them (and `Hide …` puts them back); the choice sticks for as long as the app runs. The summary counter already excluded skipped checks, so it is unchanged.
- **Identical repeated runs are collapsed into one row with a `×N` badge.** GitHub returns one check run per check suite, and a workflow triggered by `pull_request_review` gets a fresh suite on every review, so a PR could show nine identical skipped `Claude Code / claude` rows and push the interesting checks off screen. Runs are merged only when workflow, job *and* status all match — the most recent run is kept and links to its log — so a job that failed and passed on a rerun still shows as two rows. The summary counter now counts rows rather than raw runs.
- **Every GraphQL query now asks GitHub for the remaining rate-limit budget** (`rateLimit { limit remaining resetAt }` — a field that costs no points), so regular polling keeps a shared `GitHubService.LastRateLimit` current and the checks panel can pace itself against a budget it did not spend a call to learn. When the budget runs thin the panel no longer keeps a fixed 30-second pace until GitHub cuts it off: it spreads the share it may spend (20% of what is left) evenly over the time remaining in the window, slowing to at most 5 minutes between reloads, and the button shows the adjusted interval. A budget below 500 points is logged as a warning once per reset window.
